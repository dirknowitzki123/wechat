define(function () { 
	...
	function Global( vars ) {
		...
		that.layout = function() {
			...
			config.cols = cols = [];
			cols[ cols.length ] = { title: "商户名称", name: "branchName",  lockWidth: true, mouseover:true,align:'center'};
			cols[ cols.length ] = { title: "推荐码", name: "referCode",  lockWidth: true, mouseover:true,align:'center'};
			cols[ cols.length ] = { title: "经办人", name: "parReferCode",  lockWidth: true, mouseover:true,align:'center' };
			cols[ cols.length ] = { title: "账户状态", name: "status",  lockWidth: true, mouseover:true,align:'center',renderer: function( val, item, rowIndex ) {
				if(val=='10002001'){
					return "正常";
				}else{
					return "已注销";
			...
			cols[ cols.length ] = { title: "二维码下载", name: "id", lockWidth: true, mouseover:true, align:'center', renderer: function( val, item, rowIndex ) {
				return format( [ '<a href="javascript:;" class="download-qrcode"><i class="fa fa-eye"></i> 下载</a>' ] )
			}};
			...
			config.customEvents.push( {
				...
				handler: function( event, item, rowIndex ) {
					//event.data 获取参数
					that.handlers.downloadQrcode(item);
					return false;
				}
			});
			
			that.vars.typeGrid = that.selector.find( "#custGrid" ).grid( config );
		...
		handlers.exportExcel = function(){
			...
			var data = condition;
			that.loading.show();
			var url = "branch/base/info/export?"+data;
		...
		//下载二维码
		handlers.downloadQrcode = function( item ) {
    		...
    		iframeDown.src = "branch/base/info/dwloadqrcode/"+id;
